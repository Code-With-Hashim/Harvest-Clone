...
import { userSignUp } from "../redux/Actions/authenticated.action";
...
export default function Signup() {
    ...
    const [throtting, setThrotting] = useState(true)
    ...
    const handelInput = (e) => {
        ...
        if (['fullName.firstName', 'fullName.lastName'].includes(name)) {
    ...
    const handelForm = () => {
        ...
        if (throtting) {  // throttling going on
            ...
            formData.password]
                ...
                dispatch(userSignUp(formData, toast))
            }
    ...
    return (
        ...
        >
            <FormControl textAlign={'left'} py='10' w='35%' m='auto'>
                ...
                <Button onClick={handleSignIn} my='2' mt='5' bgColor={'white'} border='1px solid gray' _hover={{ border: '1px solid black' }} leftIcon={< FaGoogle />} w='full' >Sign up with Google</Button>
                ...
                <Grid rowGap={3} templateRows={'repeat(5,1fr)'} templateColumns={'repeat(3,1fr)'}>
                    ...
                    <GridItem colSpan={2}>
                        <Input onChange={handelInput} name="fullName.firstName" value={formData.fullName.firstName} border='1px solid gray' bgColor="white" type='text' isRequired />
                    </GridItem>
                    ...
                    <GridItem colSpan={2}>
                        <Input colSpan={2} onChange={handelInput} name="fullName.lastName" value={formData.fullName.lastName} border='1px solid gray' bgColor="white" type='text' />
                    </GridItem>
                    ...
                    <GridItem colSpan={2}>
                        <InputGroup>
                        <Input onChange={handelInput} type={show ? 'text' : 'password'} name='password' value={formData.password} border='1px solid gray' bgColor="white" />
                        <InputRightElement onClick={() => setShow(!show)} children={ show ? <AiOutlineEyeInvisible /> : <AiOutlineEye /> } />
                        </InputGroup>
                    </GridItem>
                </Grid>
                <Button isLoading={loading} onClick={handelForm} colorScheme='green' w='full' mt={10}>Create My Account</Button>
            </FormControl>
